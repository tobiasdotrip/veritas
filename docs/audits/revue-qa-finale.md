...
**Analyse** : Les hooks testent uniquement les états *idle* (quand `enabled: false`) et *success*. Les états *loading* (`isLoading`/`isPending`) et *error* (`isError`, `error`) ne sont pas couverts par les tests unitaires.

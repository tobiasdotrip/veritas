...
export function ComparisonTable({ result, className }: ComparisonTableProps) {
  const slugs = [
    ...
    ...result.compared.map((c) => c.slug),
  ];
  const names = [
    `${result.reference.firstName} ${result.reference.lastName}`,
    ...result.compared.map((c) => `${c.firstName} ${c.lastName}`),
  ];
    ...
    <div className={cn("overflow-x-auto", className)}>
      <table className="w-full min-w-[640px] border-collapse text-sm">
        <caption className="sr-only">
          Tableau comparatif des votes entre les députés sélectionnés
        </caption>
        <thead>
                ...
                {row.resultatGlobal ? (
                  <span
                    className={cn(
                      ...
                      row.resultatGlobal === "adopté"
                        ? "bg-success-bg text-success"
                        : "bg-danger-bg text-danger"
                    )}
                  >
                    {row.resultatGlobal === "adopté" ? "Adopté" : "Rejeté"}
                  </span>
                ) : (
                  <span className="text-xs text-text-muted">—</span>
                )}

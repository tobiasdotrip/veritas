      ...
      {data.map((d) => {
        ...
        return (
          <div key={d.key} className="flex items-center gap-3">
            <span className="w-24 shrink-0 text-xs font-medium text-text-secondary capitalize">
              {d.key}
            </span>
            <div className="h-4 flex-1 overflow-hidden rounded-full bg-neutral-bg">
              <div
                ...
                style={{
                  width: `${pct}%`,
                  backgroundColor: colors[d.key as keyof typeof colors],
                }}
              />
            </div>
...
export function VoteChart({
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-center rounded-lg border border-border bg-surface p-4",
        className,
      )}
    >
      {isDesktop ? (
